...
function showpaymentmethods() {           
	$.ajax({
		type: 'GET',
		url: site_url+"user/fn_get_payment_method_ajax",
		contentType: false,
		cache: false,
		processData:false,
		beforeSend:function(){ },                    
		success:function(data){
			...
			jQuery(".paymentid").click(function() {
		        if($(this).is(":checked")) {
		            ...
		            $.ajax({
						type: 'GET',
						url: site_url + "user/fn_set_active_payment_method",
						data:  "payment_sk="+payment_id,
						contentType: false,
						cache: false,
						processData:false,
						success:function(data){
							var arrTotal = data.split('^^');
							if(arrTotal[1] == '1'){                
				                displayToastNotificationInAjax('success','Your payment method updated to active successfully.');
				                showpaymentmethods();
				    			return false;					
							}else{
...
function deletepayment(payment_id) {
	...
	$("#payment_sk").val(payment_id);
	$.ajax({
		type: 'GET',
		url: site_url + "user/fn_delete_payment_method",
		data:  "payment_sk="+payment_id,
		contentType: false,
		cache: false,
		processData:false,
		...
		success:function(data){
			var arrTotal = data.split('^^');
			if(arrTotal[1] == '1'){                
                displayToastNotificationInAjax('success','Your payment method deleted successfully.');
                showpaymentmethods();
    			return false;					
			}else{
                displayToastNotificationInAjax('error',data);
		...
		$.ajax({
			...
			success:function(data){
				...
				if(arrTotal[1] == '1'){
					...
					jQuery("#cvc").val('');
					jQuery("#name_on_card").val('');
					
                    jQuery('#insert_payment_btn').show();
                    jQuery('#wait_insert_payment_btn').hide();
                    displayToastNotificationInAjax('success','Your payment method added successfully.');
