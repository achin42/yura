...
jQuery(document).ready(function($){
    ...
    $('#frm_client_company_detail').keydown(function(e) {
        $('#btn_client_company_detail').show();
    });
    
    $('#frm_client_company_detail').keypress(function(e) {
        var key = e.which;
        if (key == 13) {
            var client_company_detail_obj = document.getElementById("frm_client_company_detail");
            funAjaxPostClientCompanyDetail(client_company_detail_obj);
            return false;
        }
    });
    
    $("#country_code_name,#company_domain,#area_code,#company_logo_image").on("change",function() {
        $('#btn_client_company_detail').show();
    });
...
function funAjaxPostClientCompanyDetail(obj){
	jQuery('#btn_client_company_detail').hide();
	jQuery('#wait_btn_client_company_detail').show();
    ...
    //popError = false;
	if(popError === true){
		jQuery('#btn_client_company_detail').show();
        jQuery('#wait_btn_client_company_detail').hide();
		return false;
	}  else {
        			...
        			if(arrTotal[0] == 'company_exists'){
                		...
                		$.ajax({
                			...
                			error:function(jqXHR, exception){                				
                                ...
                                if(err_msg != ''){
                                    fn_send_ajax_error_handling_mail(jqXHR.status, jqXHR.responseText, exception, err_msg, 'funAjaxPostClientCompanyDetail');        
                                }
                                return false;
                			}
                		});        			 
