...
jQuery(document).ready(function($){
    ...
    $('#frm_profile_setup').keypress(function(e) {
        var key = e.which;
        if (key == 13) {
            var profile_setup_obj = document.getElementById("frm_profile_setup");
            funAjaxPostProfileSetup(profile_setup_obj);
            return false;
        }
    });
...
function back_from_company_detail_event(){
    $('#back_from_company_detail').click(function () {
        $.ajax({
            type: 'POST',
            url: '<?php echo $this->config->item("site_url");?>user/fn_get_profile_setup_ajax',
            data: '1=1',
...
function funAjaxPostProfileSetup(obj){
	jQuery('#btn_profile_setup').hide();
	jQuery('#wait_btn_profile_setup').show();
    ...
    //popError = false;
	if(popError === true){
		jQuery('#btn_profile_setup').show();
        jQuery('#wait_btn_profile_setup').hide();
		return false;
	}  else {
		...
		$.ajax({
			...
			error:function(jqXHR, exception){
                ...
                if(err_msg != ''){
                    fn_send_ajax_error_handling_mail(jqXHR.status, jqXHR.responseText, exception, err_msg, 'funAjaxPostProfileSetup');        
                }
                return false;
			}
		});
