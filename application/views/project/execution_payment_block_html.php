<div class="side-block-container payment-block-container">
    <div class="sb-head">
        <h6><span class="sb-head-icon"><img src="<?php echo $this->config->item("site_url");?>assets/images/payment-icon-white.svg" alt=""></span>Payment</h6>
            ...
            <div class="pahse-sb-upper">
                <div class="phase-form-container">
                        ...
                        <div class="form-group">
                            ...
                            <div class="position-relative" style="padding-right: 100px;">                                
                                <div class="text-box execution-text-box"><?php echo $block_data[0]->payment_amount; ?></div>
                                <span class="input-group-addon1">
                                    <div class="text-box execution-text-box"><?php echo $block_data[0]->payment_currency; ?></div>
                                </span>
                            </div>
                        </div>
                        ...
                        <div class="task-card">
                            <?php if($active_task_list_arr[$i]['action_name'] == 'pay_now' && $active_task_list_arr[$i]['payment_action'] == '1'){ ?>
                                <?php if($active_task_list_arr[$i]['block_payment_amount'] != '' && $active_task_list_arr[$i]['block_payment_amount'] > '0'){ ?>
                                    ...
                                    <div class="card-text">Pay $<?php echo number_format($active_task_list_arr[$i]['block_payment_amount'],2);?> for the Phase '<?php echo $active_task_list_arr[$i]['phase_name'];?>' by <?php echo $this->general->display_date_format_front($active_task_list_arr[$i]['block_end_date']);?></div>
                                    <div class="task-btn btn-block">
                                        <?php if($active_task_list_arr[$i]['task_done'] == '0'){ ?>
                                            <a href="javascript:void(0);" id="pay_now_<?php echo $active_task_list_arr[$i]['tasks_sk'];?>" onclick="pay_now('<?php echo $active_task_list_arr[$i]['tasks_sk'];?>','<?php echo $active_task_list_arr[$i]['block_payment_amount'];?>','<?php echo $project_sk;?>');" >pay now</a>
                                            <a href="javascript:void(0);" style="display:none;" id="unpaid_<?php echo $active_task_list_arr[$i]['tasks_sk'];?>" onclick="unpaid('<?php echo $active_task_list_arr[$i]['tasks_sk'];?>','<?php echo $active_task_list_arr[$i]['block_payment_amount'];?>','<?php echo $project_sk;?>');" >mark as unpaid</a>
                                        <?php }else{ ?>
                                            <a href="javascript:void(0);" style="display:none;" id="pay_now_<?php echo $active_task_list_arr[$i]['tasks_sk'];?>" onclick="pay_now('<?php echo $active_task_list_arr[$i]['tasks_sk'];?>','<?php echo $active_task_list_arr[$i]['block_payment_amount'];?>','<?php echo $project_sk;?>');" >pay now</a>
                                            <a href="javascript:void(0);" id="unpaid_<?php echo $active_task_list_arr[$i]['tasks_sk'];?>" onclick="unpaid('<?php echo $active_task_list_arr[$i]['tasks_sk'];?>','<?php echo $active_task_list_arr[$i]['block_payment_amount'];?>','<?php echo $project_sk;?>');" >mark as unpaid</a>
                                        <?php } ?>
                                    </div>    
                                <?php }else{ ?>
                                    <h5>Task - <?php echo $active_task_list_arr[$i]['task_type_title'];?></h5>
